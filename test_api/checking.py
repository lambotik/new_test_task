...
class Checking:
    ...
    @staticmethod
    def check_json_all_keys(result: requests.models.Response, expected_value: list):
        """
        The method gets a list of nested dictionary keys by the parent dictionary key
        :param result: requests.models.Response
        :param expected_value: list of the keys
        ...
        for i in range(1, len(response)):
            assert list(response[i]) == expected_value, \
                f"All keys is not present. {list(response[i])} is not equal {expected_value}"
